...
public class mymain {
    //源apk
    private static final String sourceApk = "force/source/" + "app-debug.apk";
    //脱壳apk的dex
    private static final String sourceDex = "force/source/" + "classes.dex";
    private static final String resultClassesDex = "force/result/" + "classes.dex";

    public static void main(String[] args) {
        try {
            //需要加壳的源APK  ，以二进制形式读出，并进行加密处理
            File srcApkFile = new File(sourceApk);
            System.out.println("apk size:" + srcApkFile.length());
            byte[] enSrcApkArray = encrpt(readFileBytes(srcApkFile));

            //需要解壳的dex  以二进制形式读出dex
            File unShellDexFile = new File(sourceDex);
            ...
            //写出新Dex
            File file = new File(resultClassesDex);
            if (!file.exists()) {
                file.createNewFile();
            }
            FileOutputStream localFileOutputStream = new FileOutputStream(resultClassesDex);
